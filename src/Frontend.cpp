...
bool Lexer::lex(std::vector<Token>&out,Diagnostic&d){
 ...
 while(i<source_.size()){
  ...
  if(std::isalpha(static_cast<unsigned char>(c))||c=='_'||c=='$'){
   ...
   auto text=source_.substr(start,i-start);const bool keyword=text=="let"||text=="const"||text=="var"||text=="true"||text=="false"||text=="null"||text=="undefined"||text=="if"||text=="else"||text=="while"||text=="for"||text=="break"||text=="continue"||text=="function"||text=="return";
...
std::unique_ptr<Expr>Parser::binary(int min){
 ...
 while(tokens_[at_].text=="("){++at_;auto call=std::make_unique<Expr>();call->kind=ExprKind::Call;call->left=std::move(left);if(tokens_[at_].text!=")")for(;;){auto arg=expression();if(!arg)return nullptr;call->arguments.push_back(std::move(arg));if(tokens_[at_].text!=",")break;++at_;}if(!consume(")","expected ')' after arguments"))return nullptr;left=std::move(call);}
...
std::unique_ptr<Expr>Parser::assignment(){
 ...
 auto left=conditional();if(!left||tokens_[at_].text!="=")return left;if(left->kind!=ExprKind::Identifier){fail(tokens_[at_],"invalid assignment target");return nullptr;}++at_;auto right=assignment();if(!right)return nullptr;auto e=std::make_unique<Expr>();e->kind=ExprKind::Assignment;e->text=left->text;e->right=std::move(right);return e;
}
