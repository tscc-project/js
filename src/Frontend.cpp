...
bool Parser::statement(Statement&s){
 ...
 if(t.text=="try"){++at_;s.kind=StatementKind::Try;if(tokens_[at_].text!="{")return fail(tokens_[at_],"expected block after try");s.try_branch=std::make_unique<Statement>();if(!statement(*s.try_branch))return false;if(tokens_[at_].text!="catch")return fail(tokens_[at_],"expected catch after try block");++at_;if(tokens_[at_].text=="("){++at_;if(tokens_[at_].kind!=TokenKind::Identifier)return fail(tokens_[at_],"expected catch binding");s.catch_name=tokens_[at_++].text;if(!consume(")","expected ')' after catch binding"))return false;}if(tokens_[at_].text!="{")return fail(tokens_[at_],"expected block after catch");s.catch_branch=std::make_unique<Statement>();return statement(*s.catch_branch);}
