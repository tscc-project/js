...
std::size_t Heap::collect(const std::vector<js_value>&roots,const std::vector<std::shared_ptr<Environment>>&environment_roots){
 ...
 fun=[&](const auto&x){if(!x||!marked.insert(x.get()).second)return;env(x->closure);obj(x->instance_prototype);obj(x->object_prototype);for(const auto&entry:x->properties)value(entry.second);};
 ...
 std::size_t reclaimed=0;for(auto&x:envs)if(!marked.count(x.get())){x->bindings.clear();x->parent.reset();++reclaimed;}for(auto&x:funcs)if(!marked.count(x.get())){x->properties.clear();x->closure.reset();x->instance_prototype.reset();x->object_prototype.reset();x->native={};++reclaimed;}for(auto&x:objs)if(!marked.count(x.get())){x->properties.clear();x->attributes.clear();x->prototype.reset();++reclaimed;}for(auto&x:arrs)if(!marked.count(x.get())){x->elements.clear();x->properties.clear();x->prototype.reset();++reclaimed;}
