...
struct Statement;
enum class ExprKind { Number,String,Boolean,Null,Undefined,Identifier,Unary,Binary,Assignment,Conditional,Function,Call };
struct Expr { ExprKind kind=ExprKind::Undefined;std::string text;double number=0;std::unique_ptr<Expr>left,right,third;std::vector<std::unique_ptr<Expr>>arguments;std::vector<std::string>parameters;std::unique_ptr<Statement>function_body; };
