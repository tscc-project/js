...
enum class ExprKind { Number,String,Boolean,Null,Undefined,Identifier,Unary,Binary,Assignment,Conditional };
struct Expr { ExprKind kind=ExprKind::Undefined;std::string text;double number=0;std::unique_ptr<Expr>left,right,third; };
enum class StatementKind { Declaration,Expression,Block,If,While,For,Break,Continue,Empty };
struct Statement {
...
class Parser {
 ...
 std::unique_ptr<Expr>expression(),assignment(),conditional(),binary(int=0),prefix();
